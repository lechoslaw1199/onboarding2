...
export default function SupportHistory() {
  ...
  const { direction, updateDirection, supportHistory, setSupportHistory } = useOnboarding();
  ...
  const handleToggle = (option) => {
    ...
    if (option === "All of the above") {
      ...
      if (newHistory.includes(option)) {
        newHistory = newHistory.filter(h => h !== option);
      } else {
  ...
  return (
    <div className="w-full flex flex-col items-center min-h-screen relative overflow-x-hidden bg-white">
      ...
      >
        <div className="w-full pt-4 pb-2 px-4">
          <h1 className="text-[24px] font-bold text-[#221750] leading-tight text-center">
            How have you supported your child with reading & writing so far?
          </h1>
        ...
        <div className="w-full flex flex-col gap-3 px-4 mb-4 mt-4">
          {options.map((option) => {
            const isSelected = supportHistory.includes(option);
            return (
              <motion.button
                key={option}
                whileTap={{ scale: 0.98 }}
      ...
      >
        <motion.button 
          whileTap={supportHistory.length > 0 ? { scale: 0.98 } : {}}
          disabled={supportHistory.length === 0}
          className={`w-full h-14 bg-[#099FF9] hover:bg-[#0088EE] text-white rounded-full text-[18px] font-bold transition-all shadow-md ${
            supportHistory.length === 0 ? 'opacity-50 cursor-not-allowed' : 'opacity-100 shadow-purple-500/20'
          }`}
