...
export default function SentencesTeaser() {
  const router = useRouter();
  const { childGender, direction, updateDirection } = useOnboarding();
  const isReady = useImagePreload("/next-step.jpeg");
    ...
    <div className="w-full flex flex-col items-center min-h-screen relative overflow-x-clip">
      ...
      >
        ...
        <h1 className="text-[24px] font-bold text-[#221750] text-start leading-tight mb-6 px-4">
          {getPossessiveCap()} next step is <span className="text-black">building sentences from words</span>
        </h1>

        <div className="text-[16px] text-start text-black font-normal mb-4 px-2 leading-relaxed">
          Through early lessons, children blend words into sentences. Step by step, they build confidence — and before long, <span className="font-bold">they&apos;re reading their first sentence</span>.
        </div>
